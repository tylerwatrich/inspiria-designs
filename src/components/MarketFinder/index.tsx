...
import { ExternalLink, MapPin, AlertTriangle, Globe, ArrowRight, ShieldCheck, TrendingUp } from 'lucide-react'

// ─── SUB-COMPONENTS ────────────────────────────────────────────────────────────

function ScoreBar({ score }: { score: number }) {
  const colorClass = score >= 85 ? 'bg-red-600' : score >= 70 ? 'bg-orange-500' : 'bg-yellow-500'
  const textColorClass = score >= 85 ? 'text-red-600' : score >= 70 ? 'text-orange-600' : 'text-yellow-600'
  ...
  return (
    <div className="flex items-center gap-3 w-full">
      ...
      <span className={cn("text-xs font-bold tabular-nums min-w-[2rem] text-right", textColorClass)}>
        {score}
      </span>
    </div>
  )
...
function TariffBadge({ risk }: { risk: TariffInfo }) {
  const styles = {
    ...
    MEDIUM: "bg-amber-50 border-amber-200 text-amber-800 dark:bg-amber-950/30 dark:border-amber-900/50 dark:text-amber-400",
  ...
  return (
    <div className={cn("mt-4 p-3 border rounded-lg flex gap-3 items-start animate-in fade-in slide-in-from-top-1 duration-300", styles[risk.level])}>
...
function MarketCard({
  ...
  return (
    <Card className="overflow-hidden border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-300 animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${index * 50}ms`, animationFillMode: 'both' }}>
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="text-center space-y-4 mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <div className="space-y-2">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-zinc-900 dark:text-zinc-50">
              Canadian Market <span className="text-red-600">Compass</span>
            </h1>
        ...
        <Card className="border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-none animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100 fill-both">
          <CardContent className="p-6 md:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="space-y-2">
                <Label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Industry / Sector</Label>
                <Select
              ...
              <div className="space-y-2">
                <Label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Your Province</Label>
                <Select
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 border-t border-zinc-100 dark:border-zinc-800">
              <div className="flex items-center space-x-3 group cursor-pointer" onClick={() => setHideUS(!hideUS)}>
                <Checkbox 
                  id="hide-us" 
                  checked={hideUS} 
                ...
                <div className="space-y-0.5">
                  <Label 
                    htmlFor="hide-us" 
                    className={cn("text-sm font-bold uppercase tracking-tight transition-colors cursor-pointer", hideUS ? "text-red-600" : "text-zinc-700 dark:text-zinc-300")}
                  >
        ...
        {hasSearched && data && (
          <div className="space-y-6">
            ...
            {/* Results Header */}
            <div id="results-header" className="flex flex-col md:flex-row md:items-end justify-between gap-4 pt-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                   <TrendingUp className="w-5 h-5 text-red-600" />
                   <h2 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-zinc-50 uppercase">{selectedIndustry}</h2>
                </div>
                <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Ranked by opportunity score · Trade volume, FTA access & Alignment</p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl">
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mr-2">Agreements:</span>
              {[
                { name: 'CUSMA', color: 'border-emerald-200 text-emerald-700 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-900/50' },
                { name: 'CPTPP', color: 'border-blue-200 text-blue-700 bg-blue-50 dark:bg-blue-950/30 dark:border-blue-900/50' },
                { name: 'CETA/CUKTCA', color: 'border-amber-200 text-amber-700 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-900/50' },
                { name: 'CKFTA', color: 'border-purple-200 text-purple-700 bg-purple-50 dark:bg-purple-950/30 dark:border-purple-900/50' },
              ].map((ta) => (
                <span key={ta.name} className={cn("text-[10px] font-bold px-2 py-0.5 rounded-sm border uppercase tracking-tight", ta.color)}>
                  {ta.name}
                </span>
              ))}
            ...
            {visibleMarkets.length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-zinc-900/30 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl space-y-4">
                ...
                <div className="space-y-1">
                  <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-tight">No Markets Found</h3>
                  <p className="text-sm text-zinc-500">All opportunities were filtered out. Try disabling the US filter.</p>
                </div>
              </div>
            ) : (
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
                  <div className="max-w-xl space-y-4 text-center md:text-left">
                    ...
                    <p className="text-zinc-400 text-base leading-relaxed">
                      We help Canadian exporters build the digital presence and automated systems needed to compete globally. Whether pivoting from the US or scaling domestically, we provide the technical edge.
                    </p>
                    <div className="pt-2">
                  ...
                  <div className="flex flex-col gap-3 w-full sm:w-auto min-w-[240px]">
                    <Button asChild className="h-14 bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl shadow-red-600/20 active:scale-95 transition-all">
                      <a href="https://inspiria.ca/contact" target="_blank" rel="noopener noreferrer">
                        Book Strategy Call <ArrowRight className="ml-2 w-4 h-4" />
                      </a>
                    </Button>
                    <Button variant="outline" asChild className="h-14 border-zinc-700 text-zinc-400 hover:text-white hover:bg-zinc-800 font-bold uppercase tracking-widest text-[10px] rounded-2xl active:scale-95 transition-all">
                      <a href="https://inspiria.ca" target="_blank" rel="noopener noreferrer">
                        Explore Our Work
                      </a>
                    </Button>
                  </div>
            ...
            <footer className="mt-12 p-6 border border-zinc-200 dark:border-zinc-800 rounded-2xl bg-white dark:bg-zinc-900/30">
              <p className="text-[11px] text-zinc-500 leading-relaxed italic">
                <strong className="text-zinc-700 dark:text-zinc-300 not-italic uppercase tracking-tighter mr-1">Trade Intelligence Disclaimer:</strong>
                Opportunity scores reflect relative market potential based on early 2025 trade volume, FTA access, and demand trends. This is not financial or legal advice. Conditions may shift rapidly. Always verify with official sources: 
                <a href="https://www.tradecommissioner.gc.ca" target="_blank" rel="noopener noreferrer" className="mx-1 text-red-600 hover:underline inline-flex items-center gap-0.5">Trade Commissioner Service <ExternalLink className="w-2.5 h-2.5" /></a> and 
                <a href="https://www.international.gc.ca" target="_blank" rel="noopener noreferrer" className="ml-1 text-red-600 hover:underline inline-flex items-center gap-0.5">Global Affairs Canada <ExternalLink className="w-2.5 h-2.5" /></a>.
              </p>
            </footer>
          </div>
        )}
        ...
        {!hasSearched && (
          <div className="text-center py-24 space-y-6 animate-in fade-in zoom-in-95 duration-1000">
             ...
             <div className="space-y-2">
                <h3 className="text-zinc-400 font-bold uppercase tracking-[0.3em] text-xs">Awaiting Input</h3>
                <p className="text-zinc-500 text-sm italic">Select your industry above to begin identifying opportunities.</p>
             </div>
          </div>
        )}
      </div>
